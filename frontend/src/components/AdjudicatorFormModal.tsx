...
import type { AdminAdjudicator, Region } from "@/lib/api/admin";

interface Props {
  open: boolean;
  initial?: AdminAdjudicator | null;
  onClose: () => void;
  onSubmit: (a: Partial<AdminAdjudicator>) => void;
  submitting?: boolean;
}

const REGIONS: Region[] = ["Europe", "Americas", "Asia-Pacific", "MEA"];
...
type FormState = Omit<AdminAdjudicator, "id" | "user_id" | "created_at" | "updated_at"> & { id?: string };
...
export default function AdjudicatorFormModal({ open, initial, onClose, onSubmit, submitting }: Props) {
    ...
    <Modal
      ...
      footer={
        <>
          ...
          <Button
            onClick={() => valid && onSubmit({ ...form, initials: initials(form.name) })}
            disabled={!valid || !!submitting}
          >
            {submitting ? "Saving…" : isEdit ? "Save changes" : "Add adjudicator"}
          </Button>
        </>
      }
    >
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <Field label="Home city">
          <input className="input" value={form.home_city ?? ""} onChange={(e) => set("home_city", e.target.value)} />
        </Field>
        <Field label="Home country">
          <input className="input" value={form.home_country ?? ""} onChange={(e) => set("home_country", e.target.value)} />
        </Field>
        ...
        <Field label="Years experience">
          <input type="number" min={0} className="input" value={form.years_experience} onChange={(e) => set("years_experience", Number(e.target.value))} />
        </Field>
        <Field label="Rating (0–5)">
          <input type="number" step="0.1" min={0} max={5} className="input" value={form.rating} onChange={(e) => set("rating", Number(e.target.value))} />
        </Field>

        <TagField full label="Specialties" placeholder="Add a specialty (Enter)"
        ...
        <TagField full label="Languages" placeholder="Add a language (Enter)"
          value={langInput} onChange={setLangInput}
          tags={form.languages}
          onAdd={() => addTo("languages", langInput, () => setLangInput(""))}
          onRemove={(v) => removeFrom("languages", v)} />
        <TagField full label="Certifications" placeholder="Add a certification (Enter)"
...
function TagField({
  ...
  tags: string[]; onAdd: () => void; onRemove: (v: string) => void; full?: boolean;
}) {
  return (
