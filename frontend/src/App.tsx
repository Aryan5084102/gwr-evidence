...
import { useAppSelector, useAppDispatch, login, logout, authUserFromBackend } from "@/redux/store";
...
/**
 * On app boot, if we have an access token but no Redux user (e.g. after a hard
 * reload), call /auth/me to rehydrate the session. If the token is invalid,
 * clear Redux state too so the login page shows.
 */
function useSessionRehydrate() {
  ...
  useEffect(() => {
    ...
    if (!tok) {
      // Stale Redux state without a token → force logout
      if (user) dispatch(logout());
      return;
    }
    if (user) return; // already hydrated
    authApi.me()
      .then((me) => dispatch(login(authUserFromBackend(me))))
