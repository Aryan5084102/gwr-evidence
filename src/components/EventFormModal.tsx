...
function newId() {
  ...
  return `GWR-${year}-${String(Math.floor(Math.random() * 9000) + 1000)}`;
}

function isoToInput(iso: string): string {
  if (!iso) return "";
  // Convert ISO to value usable by <input type="datetime-local"> (UTC)
  const d = new Date(iso);
  const pad = (n: number) => String(n).padStart(2, "0");
  return `${d.getUTCFullYear()}-${pad(d.getUTCMonth() + 1)}-${pad(d.getUTCDate())}T${pad(d.getUTCHours())}:${pad(d.getUTCMinutes())}`;
}

function inputToIso(v: string): string {
  if (!v) return "";
  // Treat <input> value as UTC
  return new Date(v + ":00Z").toISOString();
}

export default function EventFormModal({ open, initial, defaultGeofenceM, onClose, onSubmit }: Props) {
  ...
  const [form, setForm] = useState<AdminEvent>(() => initial ?? {
    ...
    lat: 51.5,
    lon: -0.12,
    startISO: new Date(Date.now() + 14 * 86400000).toISOString(),
    endISO: new Date(Date.now() + 14 * 86400000 + 3 * 3600000).toISOString(),
    status: "Draft",
    priority: "Standard",
    participantCount: 1,
  ...
  function set<K extends keyof AdminEvent>(k: K, v: AdminEvent[K]) {
    setForm((f) => ({ ...f, [k]: v }));
  }
  ...
  return (
    <Modal
      open={open}
      onClose={onClose}
      title={isEdit ? `Edit event · ${form.id}` : "Create new record attempt event"}
    ...
    >
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <Field label="Geo-fence radius (m)">
          <input type="number" min={50} step={50} className="input" value={form.geofenceRadiusM} onChange={(e) => set("geofenceRadiusM", Number(e.target.value))} />
        </Field>

        <Field label="Starts (UTC)">
          <input type="datetime-local" className="input" value={isoToInput(form.startISO)} onChange={(e) => set("startISO", inputToIso(e.target.value))} />
        </Field>
        <Field label="Ends (UTC)">
          <input type="datetime-local" className="input" value={isoToInput(form.endISO)} onChange={(e) => set("endISO", inputToIso(e.target.value))} />
        </Field>
        ...
        <Field label="Participants">
          <input type="number" min={1} className="input" value={form.participantCount} onChange={(e) => set("participantCount", Number(e.target.value))} />
        </Field>
        <Field label="Required adjudicators">
          <input type="number" min={1} className="input" value={form.requiredAdjudicators} onChange={(e) => set("requiredAdjudicators", Number(e.target.value))} />
        </Field>
